...
function validateBossRules(levelConfig) {
    const aggression = levelConfig.boss.aggression;
    const message = `Level ${levelConfig.number}: boss.aggression must not exceed 1.`;
    assertLevelConfig(aggression <= 1, message);
}
